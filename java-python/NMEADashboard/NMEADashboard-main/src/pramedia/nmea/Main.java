...
public class Main extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        btnInfo.setIcon(new javax.swing.ImageIcon(getClass().getResource("/pramedia/nmea/globe.png"))); // NOI18N
                ...
                protected Boolean doInBackground() {
                    ...
                    activePort.addDataListener(new com.fazecast.jSerialComm.SerialPortDataListener() {
                        ...
                        public void serialEvent(com.fazecast.jSerialComm.SerialPortEvent event) {
                            ...
                            if (numRead > 0) {
                                // Konversi byte ke string dan cetak ke log
    ...
    private void parseNMEA(String nmeaSentence) {
        // 1. Bersihkan spasi/karakter tak terlihat dan pastikan diawali tanda $
        String sentence = nmeaSentence.trim();
        if (!sentence.startsWith("$")) {
            return;
        }

        // 2. Pisahkan data berdasarkan tanda koma
        String[] tokens = sentence.split(",");

        // 3. Ambil Header identitas NMEA (misal: $GPGGA, $GPRMC, $GNGGA, dll)
        ...
        try {
            ...
            if (header.endsWith("GGA")) {
                if (tokens.length > 9) {
                    ...
                    String lat = tokens[2] + " " + tokens[3]; // Contoh: 0714.590 S
                    String lon = tokens[4] + " " + tokens[5]; // Contoh: 11246.139 E
                    String fix = tokens[6]; // 1 = Fix, 0 = No Fix
                    String sat = tokens[7]; // Jumlah Satelit
                    String alt = tokens[9] + " Meter"; // Ketinggian

                    System.out.println("Satelit : " + sat);
                }
                ...
                if (tokens.length > 9) {
                    ...
                    if (status.equals("A")) {
                        String lat = tokens[3] + " " + tokens[4];
                        String lon = tokens[5] + " " + tokens[6];
                        String speed = tokens[7] + " Knots";
                        String date = tokens[9]; // Format: DDMMYY
            ...
            lbError.setText("Gagal parsing baris: " + sentence);
        }
    }
